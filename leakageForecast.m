...
rock2D.perm = ones(Gt.cells.num,1) * 300 * milli*darcy;
...
[Gt, rock2D] = getFormationTopGrid('johansenfm',3);
...
ta               = trapAnalysis(Gt, true);
seainfo          = getSeaInfo('johansenfm', 760);
...
wcinx = findEnclosingCell(Gt, [5.4e5, 6e6]);
W = addWell([], Gt.parent, rock2D, wcinx, ...
                'name',     ['W' num2str(wcinx)],  ...
                'Type',     'rate', ...
                'Val',      0.3536, ...
                'comp_i',   [0,1], ...
                'Radius',   0.3, ...
                'refDepth', Gt.parent.cells.centroids(wcinx,3) );
